...
class ExperimentRecordTests(unittest.TestCase):
    ...
    def test_matrix_points_to_reviewed_summaries_and_closes_r1(self):
        ...
        opd_summary = load_json(ROOT / opd["evidence"])
        self.assertEqual(opd_summary["status"], "failed")
        self.assertEqual(opd_summary["runtime"]["student_exit_code"], 0)
        ...
        self.assertTrue((ROOT / opd["qualification_gate_v1"]).is_file())
        self.assertTrue((ROOT / opd["prompt_smoke_gate_v2"]).is_file())
        self.assertTrue((ROOT / opd["qualification_gate_v2"]).is_file())
    ...
    def test_public_docs_keep_external_and_measurement_boundaries_explicit(self):
        ...
        self.assertIn("training-only smoke", model_card)
