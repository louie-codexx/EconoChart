...
> 当前状态（2026-08-29）：正式 SFT 与 3,600 步 GRPO 均已完成并通过 adapter、重载和 2,496 条内部测试审计。GRPO 相对 SFT 的 overall 成对变化为 `-0.000157`，95% CI 为 `[-0.002294, 0.002112]`，所有分项均无统计显著差异，因此不宣称 GRPO 有独立边际增益；Base 到 GRPO 的 `+0.456563` 是 SFT 与 GRPO 的累计提升，不能全部归因于 GRPO。ChartQA、ChartQAPro 和定性非退化检查仍待完成。
