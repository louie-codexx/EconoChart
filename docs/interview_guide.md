...
> EconoChart 是一个基于 Qwen3-VL-4B-Instruct 的数字经济经营图表后训练项目。我的核心工作不是简单调用 LoRA，而是解决了三个问题：第一，构建企业级无泄漏、数值一致且无标签捷径的多视图经营数据；第二，在单卡约束下用 QLoRA SFT 学习可核验的结论—证据—风险—建议结构；第三，把数值、趋势、证据和风险编码成可单测 reward，从通过评测的 SFT adapter 继续完成 3,600-step GRPO，共 14,400 次 rollout。固定 2,496 条内部测试上，Base overall 为 `0.349780`，最终 GRPO 为 `0.806343`，累计提升 `+0.456563`；GRPO 阶段在 504 个 adapter 张量全部发生更新后仍守住强 SFT 的总体能力，SFT→GRPO delta `-0.000157`、95% CI `[-0.002294, 0.002112]`，没有统计可确认的退化。固定 4,446 条 ChartQA/ChartQAPro 外评上，三模型结论是 `NO_EXTERNAL_GAIN`。并行 mixed-SFT 虽将 ChartQA exact 提升 `+0.062000`，却未通过 ChartQAPro relaxed 非劣门，因此不晋升。最后，我在 1,171 条 MME-Finance 开放题上完成 Base→mixed-SFT 配对审计：ANLS、token-F1 和约 6.16 倍吞吐改善，但数值召回由 `0.699934` 降至 `0.282634`。这定位出“短答更快但数值覆盖不足”的下一优化目标，也说明我既能落地完整 RL 流水线，也能用 guardrail 和负结果约束下一轮优化。
...
面试时可以先用更有力但仍准确的一句话回答：**“我把一个 `0.349780` 的 Base 训练成 `0.806343` 的最终 GRPO 工件，并完成 14,400 次 rollout、六类可单测 reward、checkpoint 恢复和成对统计审计；RL 阶段没有破坏强 SFT 已获得的总体能力。”** 如果面试官继续追问，再补充累计增益不能全部归因于 GRPO，体现自己理解消融和因果归因。
