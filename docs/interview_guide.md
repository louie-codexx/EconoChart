...
这份材料是讲解框架，不是背诵稿。已经完成的内部指标使用审计值；ChartQA/ChartQAPro 模型外评和定性案例仍明确标为待完成，不用内部结果代填。

## 1. 三分钟项目介绍

> EconoChart 是一个基于 Qwen3-VL-4B-Instruct 的数字经济经营图表后训练项目。我的核心工作不是简单调用 LoRA，而是解决了三个问题：第一，构建企业级无泄漏、数值一致且无标签捷径的多视图经营数据；第二，在单卡约束下用 QLoRA SFT 学习可核验的结论—证据—风险—建议结构；第三，把数值、趋势、证据和风险编码成可单测 reward，从通过评测的 SFT adapter 继续做 GRPO。固定 2,496 条内部测试上，Base overall 为 `0.349780`，正式 SFT 为 `0.806499`，Base→SFT 成对提升 `+0.456719`、95% CI `[0.448794, 0.464756]`。GRPO 为 `0.806343`，相对 SFT 的变化只有 `-0.000157`、95% CI `[-0.002294, 0.002112]`，所以不宣称 GRPO 有独立边际增益。当前局限是 ChartQA/ChartQAPro 模型外评与定性失败案例仍未封板，合成内部结果不能外推为真实业务泛化。
...
| 模型 | Internal overall | Numeric | Trend | Risk | ChartQA | ChartQAPro | 显存证据 | 训练时长 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Base | 0.349780 | 0.240061 | 0.643119 | 0.059233 | 待模型外评 | 待模型外评 | 未按统一整卡峰值归档 | - |
| SFT QLoRA | 0.806499 | 0.594490 | 0.956881 | 0.685105 | 待模型外评 | 待模型外评 | Trainer 峰值分配增量 2,790 MiB，非整卡峰值 | 26,816.85s（约 7:26:57） |
| SFT + GRPO | 0.806343 | 0.593353 | 0.958716 | 0.689895 | 待模型外评 | 待模型外评 | `nvidia-smi` 单次进程观测 7,614 MiB，非峰值 | 14,168.06s（Trainer 结果；不等于含中断的总计费墙钟） |

Base→SFT 的提升可靠；SFT→GRPO 所有分项均为 inconclusive，不能把 Base→GRPO 的累计提升归给 GRPO。还需从冻结预测中完成定性非退化复核，并准备一个视觉读取错误、一个算术/趋势错误、一个建议与证据不一致案例；这三类案例未审核前继续标为待完成。
