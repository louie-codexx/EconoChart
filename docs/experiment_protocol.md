...
执行偏差审计（2026-08-29）：正式 R1 在内部 SFT 评测、adapter 重载和 reward 工程门通过后启动，但当时 ChartQA、ChartQAPro 与定性失败案例护栏尚未完成。这不影响内部 SFT→GRPO 配对比较本身，但意味着 R1 只能作为内部审计的探索实验，不能据此发布外部分布或最终产品结论。ChartQA/ChartQAPro 与短答案错误迁移已于 2026-08-30 补齐并固定为 `NO_EXTERNAL_GAIN`：Base→SFT 出现可靠外部退化，SFT→GRPO 未建立修复；开放式长报告的建议—证据人工非退化仍须单独完成。这些事后证据不能追溯改写为“R1 启动前已通过”。
